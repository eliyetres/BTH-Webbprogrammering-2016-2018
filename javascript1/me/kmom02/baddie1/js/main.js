...
(function(){
	...
	document.addEventListener("keydown", function(event) {
		...
		if (key===37){
			...
			document.getElementById("baddie").className += " baddieflip";
			...
			document.getElementById("baddie").className = "baddie";
				...
				if(baddie.offsetLeft > wi){
				left =baddie.style.left = parseInt(wi);
			}
		}
		else if (key===40){
			top = baddie.style.top = parseInt(baddie.style.top) + step;

			if(baddie.offsetTop > hi){
				top =baddie.style.top = parseInt(hi);
			}
